Plot2 <- function() {
  ...
  # Separating NEI into data from Baltimore City in the years 1999, 2002, 2005, 2008
  data_1999 <- subset(NEI, NEI$year == 1999 & NEI$fips == "24510")
  data_2002 <- subset(NEI, NEI$year == 2002 & NEI$fips == "24510")
  data_2005 <- subset(NEI, NEI$year == 2005 & NEI$fips == "24510")
  data_2008 <- subset(NEI, NEI$year == 2008 & NEI$fips == "24510")
  
  # Finding the total pollution in Baltimore City for 1999, 2002, 2005, 2008 in kilotons
  ...
  barplot(height = total_pollution, names.arg = year, xlab = "Year", ylab = "Total Pollution (Kilotons)", main = "Total Pollution vs. Year in Baltimore City", ylim = c(0, 4))
